...
public class AppUtil : MonoBehaviour
{
    ...
    public static Tween MoveRect(RectTransform rect, string from, bool isMoveIn, float duration, string ease, float delay=0f){
        /* from："上" */
        Ease easeType = (Ease)Enum.Parse(typeof(Ease), ease);
        Vector2  targetAnchoredPos = rect.anchoredPosition;
        switch(from){
            case "上":
            if(!isMoveIn) targetAnchoredPos = new Vector2(targetAnchoredPos.x, 800f);
            if(isMoveIn) rect.anchoredPosition = new Vector2(targetAnchoredPos.x, 800f);
            break;
        }
